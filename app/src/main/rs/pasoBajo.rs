...
uchar4 RS_KERNEL pasobajo(uchar4 in, uint32_t x, uint32_t y) {
    ...
    in.r = (int) 0.1*pr.topLeft.r + 0.1*pr.top.r + 0.1*pr.topRight.r + 0.1*pr.left.r + 0.1*pr.center.r + 0.1*pr.right.r + 0.1*pr.bottomLeft.r + 0.1*pr.bottom.r + 0.1*pr.bottomRight.r;
    in.g = (int) 0.1*pr.topLeft.g + 0.1*pr.top.g + 0.1*pr.topRight.g + 0.1*pr.left.g + 0.1*pr.center.g + 0.1*pr.right.g + 0.1*pr.bottomLeft.g + 0.1*pr.bottom.g + 0.1*pr.bottomRight.g;
    in.b = (int) 0.1*pr.topLeft.b + 0.1*pr.top.b + 0.1*pr.topRight.b + 0.1*pr.left.b + 0.1*pr.center.b + 0.1*pr.right.b + 0.1*pr.bottomLeft.b + 0.1*pr.bottom.b + 0.1*pr.bottomRight.b;
...
void process(rs_allocation inputImage, rs_allocation outputImage) {
    ...
    rsForEach(pasobajo, current_alloc, current_alloc);
