...
uchar4 RS_KERNEL roberts(uchar4 in, uint32_t x, uint32_t y) {
    ...
    in.r = bound255(abs(pr.center.r - pr.bottomRight.r) + abs(pr.right.r - pr.bottom.r));
    in.g = bound255(abs(pr.center.g - pr.bottomRight.g) + abs(pr.right.g - pr.bottom.g));
    in.b = bound255(abs(pr.center.b - pr.bottomRight.b) + abs(pr.right.b - pr.bottom.b));
...
void process(rs_allocation inputImage, rs_allocation outputImage) {
    ...
    rsForEach(roberts, current_alloc, current_alloc);
